...
- `/sale/xa-hang-le-size` also lists a product running a valid active `FLASH_SALE` when its stock proves it is "lẻ size" (owner decision, 2026-09-26). The proof is the `Lẻ size - Chỉ còn ít` stock proof below, read from the sale projection: at least one size sold out (every mapped, unambiguous option of that size has no stock left; a sold-out colour of a size still selling in another colour does not count), at least one option still purchasable, and fewer than **10** ready pieces left across the purchasable options. Stock is read as the capacity authority reads it: only a safe-integer count proves anything, so a fractional or out-of-range mirrored quantity (refused at checkout as `invalid-stock`) counts neither as sold out nor as for sale, and only a Flash variant that would actually sell may represent or price the product there. Only a product selling as `STANDARD` can prove it; under `OVERSELL`/`PREORDER` zero stock does not mean sold out. A composite (FULL SET) product is never auto-admitted: its capacity is derived from its components, not from the parent stock this proof reads, so it fails closed until the proof reads component-derived capacity (an explicit `CLEARANCE` campaign still lists it). The campaign stays a Flash Sale: the product remains on `/sale/flash-sale`, and on both listings its card shows the Flash price, `FLASH SALE` badge and countdown — it is not re-labelled `CLEARANCE` and does not take the `Lẻ size - Chỉ còn ít` tag, which still needs a `CLEARANCE` discount.
...
- rendered-quote proof input larger than `MAX_RENDERED_QUOTE_PROOF_BYTES` is rejected before decode/MAC verification and follows the normal refreshed-quote/no-submit failure path;
...
- atomically advance the durable promotion pricing revision in the same transaction on successful effective mutation.

**Disable/end-early is different:** it does not need to prove pricing coverage or acquire the expanded variant lock set. After authenticating/authorizing and locking the campaign row, it must be able to disable the campaign in bounded work even if PRODUCT coverage is currently >2000. A successful Disable/end-early still advances the durable promotion pricing revision in the same campaign transaction. Rollback safety takes precedence over re-running activation validation.

**Copy is different:** copy snapshots campaign fields + explicit target rows only. It does not expand PRODUCT coverage or reject because current coverage is >2000; resulting Draft health may report expansion-limit invalidity. Copy does not advance the promotion pricing revision merely by existing as Draft.
...
Sale sub-listings (PR #79): `/sale` lists every kind; `/sale/uu-dai`, `/sale/flash-sale` and `/sale/xa-hang-le-size` narrow membership, pricing, tracking and refresh to `PROMOTION`, `FLASH_SALE` (with the full-window invariant below) and `CLEARANCE` respectively. A card on a sub-listing never shows a discount from another kind. The one exception is the stock-proven Flash Sale admission to `/sale/xa-hang-le-size` above: on exactly the admitted products that listing also shows (and tracks) the `FLASH_SALE` price, and it refreshes on Flash Sale boundaries too. A product listed there for `CLEARANCE` alone never shows a Flash sibling's price.
...
- indexes support campaign/target identity, lifecycle/time, overlap lookups, and the durable singleton/equivalent promotion-pricing revision;
...
- Merchant cache hits may perform one bounded cheap durable-revision read; heavy feed regeneration remains collapsed/single-flight and bounded by #153 M4.
