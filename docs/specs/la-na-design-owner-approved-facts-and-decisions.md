...
| `pancakeChatPageId` | `web_lanadesign` | **approved.** Pancake website Chat Plugin page id, supplied by the repository owner on 2026-09-26 from Pancake › Chat Plugin › installation code, in Claude Code session [`session_017vxPKk3MTUMv7D21UQJ7Ms`](https://claude.ai/code/session_017vxPKk3MTUMv7D21UQJ7Ms). When set, the storefront loads this chat widget **in place of** the Messenger link button (owner decision, same session), only when the page is served on a host on the plugin's Pancake domain list, matched exactly in the browser: `www.lanadesign.vn` and the temporary production host `la.lanadesign.vn` (the owner set the list to `la.lanadesign.vn` and `lanadesign.vn` on 2026-09-26; both storefront hosts were verified then to pass Pancake's domain check with the live script; the bare apex `lanadesign.vn` is not a storefront host, ADR 0010). Any other host, staging, CI, local builds, or an unset id keep the derived Messenger link button above |
