...
- [ ] T2 Implement below-`lg` swipe gallery + index + lightbox + information order; assert the same PDP contract at 390px and 768px.
...
- [ ] T6 Reorder mobile checkout summary/form/totals/preorder-notice/submit without duplicating checkout authority; pin `N = sum(line.quantity)` and preserve existing preorder/mixed-order notice before submit.
