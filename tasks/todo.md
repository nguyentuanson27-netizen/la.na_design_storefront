...
>
> Status: **3 of 5 accepted** (G4 migration path, G5 architecture, G3 transport). Checkpoint B is
> **not** closed while G1 and G2 acceptance remain open.
...
- [ ] G1 Merchant + structured-data mapping accepted. **Still open.** The G1 gate work is done (ADR 0011), but no explicit owner acceptance *for Checkpoint B* exists in the repository or in session evidence. Only that acceptance step is outstanding — no further investigation is required.
- [ ] G2 Pancake/composite evidence accepted. **Still open**, on the same terms: the probe is complete and recorded, and it does **not** need re-running. What is missing is only the explicit owner acceptance.
