...
- [x] **I3** `ProductSellingPolicyEditor` on the product editor: a radio **fieldset** for the three mutually exclusive modes (a grouped choice is what a screen reader must hear before the options) plus the negative limit, over I2's `requireAdminSession` boundary. Reads through `resolveSellingPolicy()` so the surface shows the same missing-row answer every consumer gets, and shows **chưa cấu hình** distinctly from **đã cấu hình** (§5.1's `isDefault`) with a clear-to-unconfigured action, since without one an operator could never undo a review. Each refusal reason gets its own Vietnamese message — being told only "không hợp lệ" leaves an operator guessing which of three fields the boundary objected to — and the §11 composite refusal surfaces as a readable reason instead of a silent no-op. Driven end to end in the admin a11y runtime: an owner's `OVERSELL`/`−5` lands in `ProductSellingPolicy`, clearing removes it, a set is refused, and the form scans with zero Axe violations. **No migration**; the table is I1's.
